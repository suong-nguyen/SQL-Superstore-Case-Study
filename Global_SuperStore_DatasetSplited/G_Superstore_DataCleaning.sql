...
--- Show missing data
	...
	SELECT 'Time' AS Table_Name,
		   SUM(CASE WHEN TimeID IS NULL THEN 1 ELSE 0 END) AS Null_TimeID,
		   ...
		   SUM(CASE WHEN OrderDay IS NULL THEN 1 ELSE 0 END) AS Null_OrderDay,
		   SUM(CASE WHEN OrderMonth IS NULL THEN 1 ELSE 0 END) AS Null_OrderMonth,
		   SUM(CASE WHEN OrderYear IS NULL THEN 1 ELSE 0 END) AS Null_OrderYear,
		   SUM(CASE WHEN ShipDay IS NULL THEN 1 ELSE 0 END) AS Null_ShipDay,
		   SUM(CASE WHEN ShipMonth IS NULL THEN 1 ELSE 0 END) AS Null_ShipMonth,
		   SUM(CASE WHEN ShipYear IS NULL THEN 1 ELSE 0 END) AS Null_ShipYear
	FROM Time
...
/* 2 - Remove duplicate data */
	...
	UNION ALL
	SELECT 'Time' AS Table_Name,
		   COUNT(*) - COUNT(DISTINCT TimeID) AS Duplicate_Count
	FROM Time
	UNION ALL
	SELECT 'Orders' AS Table_Name,
